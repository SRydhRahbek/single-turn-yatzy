...
impl Hand {
    ...
    pub fn evaluate(&self, board: &Board) -> (u32, Option<Category>) {
        ...
        //Nothing beats Yatzy
        if board.0.get(&Category::Yatzy).expect("Missing Yatzy").is_none() {
            if self.eval_yatzy() == 50 {
                return (50, Some(Category::Yatzy));
            }
        }

        if board.0.get(&Category::Chans).expect("Missing Chans").is_none() {      //TODO: rethink this
        ...
        if board.0.get(&Category::Sexor).expect("Missing Sexor").is_none() && best_value < 30 {
        ...
        if board.0.get(&Category::Femmor).expect("Missing Femmor").is_none() && best_value < 25 {
        ...
        if board.0.get(&Category::Fyrtal).expect("Missing Fyrtal").is_none() && best_value < 24 {
        ...
        if board.0.get(&Category::Tvapar).expect("Missing Tvåpar").is_none() && best_value < 22 {
        ...
        if board.0.get(&Category::StorStraight).expect("Missing Stor Straight").is_none() && best_value < 20 {
        ...
        if board.0.get(&Category::Fyror).expect("Missing Fyror").is_none() && best_value < 20 {
        ...
        if board.0.get(&Category::Tretal).expect("Missing Tretal").is_none() && best_value < 16 {
        ...
        if board.0.get(&Category::LitenStraight).expect("Missing Liten Straight").is_none() && best_value < 15 {
        ...
        if board.0.get(&Category::Treor).expect("Missing Treor").is_none() && best_value < 15 {
        ...
        if board.0.get(&Category::Tvaor).expect("Missing Tvåor").is_none() && best_value < 10 {
        ...
        if board.0.get(&Category::Ettor).expect("Missing Ettor").is_none() && best_value < 5 {
    ...
    fn eval_ettor(&self) -> u32 {self.count_instances(1)}
    fn eval_tvaor(&self) -> u32 {self.count_instances(2)*2}
    fn eval_treor(&self) -> u32 {self.count_instances(3)*3}
    fn eval_fyror(&self) -> u32 {self.count_instances(4)*4}
    fn eval_femmor(&self) -> u32 {self.count_instances(5)*5}
    fn eval_sexor(&self) -> u32 {self.count_instances(6)*6}
